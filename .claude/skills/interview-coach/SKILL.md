...
Don't invent specifics. If the user is vague, mark the field `TBD — user to fill` and move on. A half-drafted story in the bank is still an asset; a fabricated one is a landmine.
...
- **Fabricating story details.** If the user is vague on a metric or outcome, mark the field `TBD` rather than inventing plausible-sounding numbers. A single fabricated claim can blow up an interview when the interviewer probes.
