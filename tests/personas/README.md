---
purpose: Persona fixtures for agent-driven testing of job-hunt-skills skills
---
...
5. **Target role** — a paste-in job posting the persona is applying to. Used by `resume-tailor`, `interview-coach`, `company-research`.
...
Repo-level skill contract checks do run automatically:

```bash
python scripts/test_skill_contracts.py
```

Those tests do not grade output quality. They catch structural regressions such as missing skills, stale report frontmatter, story-bank schema drift, and resume/CV format handling.
...
| [aisha-okonkwo-academic-to-industry.md](aisha-okonkwo-academic-to-industry.md) | UK postdoc moving to industry data science (London, hybrid) | resume-builder (CV mode), resume-tailor, company-research, resume-auditor |
