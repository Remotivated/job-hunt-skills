...
Invoke `resume-drift-check` in initial-build mode with the generated content and current interview conversation as evidence.
...
In single-document modes, pass only the file written. The script writes `.docx` next to each input and creates `.pdf` when LibreOffice is available.
...
See `guides/resume-philosophy.md` and `guides/ats-myths.md`.
