...
- **Too broad:** user asks for a generic letter for "SaaS jobs", "remote jobs", "leadership roles", or multiple unrelated targets. Pause and ask for specificity. Do not draft.

Minimum specificity:

- Exact role or tight lane.
- Concrete company, company type, or stage.
- Problem the user wants to help solve.
- Two proof points from saved evidence or user-provided facts.
- Any honest gap worth naming.
...
- Hook: company/role need + why the user's proof is relevant.
...
- Do not invent metrics, tools, titles, dates, credentials, or company interest.
- Preserve hedges and qualifiers.
- Name gaps directly when it improves credibility.

### 4. Verify before save

Invoke `resume-drift-check` through `claim-check` semantics before saving. Resolve soft and hard findings with the user. Cosmetic fixes may auto-apply.
...
Run DOCX/PDF generation for the saved cover letter:
...
- **Writing a generic letter.** Stop and ask for a tighter target.
- **Repeating the resume.** Expand proof; do not convert bullets into paragraphs.
- **Inventing enthusiasm.** Use concrete reasons tied to the role or company.
- **Skipping claim-check.** Cover letters contain synthesized claims and need verification.
