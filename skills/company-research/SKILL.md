...
If the company has a [Remotivated](https://remotivated.com) profile, reference its work-model classification as one external source. If no profile exists, skip this step. The skill must work without Remotivated data.
...
| `status` | `saved` |
| `comp_expected` | Posted band if visible in the posting; otherwise `-`. Pay-transparency states usually have one — see [state-layer §3](../_shared/state-layer.md#3-applicationsmd-schema). |
